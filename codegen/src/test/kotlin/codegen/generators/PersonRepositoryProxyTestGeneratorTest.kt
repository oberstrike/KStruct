...
class RepositoryProxyTest(
    ...
    fun findByName(name: String): PersonDTO = helper.toDTO { helper.compute { personRepository.findByName(name) } }

    fun isDeleted(id: Long): Boolean = helper.compute { personRepository.isDeleted(id) }!!

    fun save(person: PersonDTO) = helper.toDTO { helper.compute { personRepository.save(helper.toModel { person }) } }

    fun getAll(persons: List<PersonDTO>) = helper.toDTOs { helper.compute { personRepository.getAll(helper.toModels { persons }) } }
}
